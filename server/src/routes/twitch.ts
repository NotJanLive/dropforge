...
router.get("/status", requireUser, async (req, res) => {
  ...
  const linked = isTwitchLinked(user.id);
  const session = linked ? await getTwitchSession(user.id) : null;
  res.json({
    linked,
...
router.get("/miner/settings", requireUser, (req, res) => {
  ...
  res.json({
    ...
    priorityGames: row ? JSON.parse(row.priorityGames) : [],
    excludeGames: row ? JSON.parse(row.excludeGames) : [],
...
router.put("/miner/settings", requireUser, async (req, res) => {
  ...
  await minerManager.updateSettings(user.id, {
    priorityMode: priorityMode as "PRIORITY_ONLY" | "ENDING_SOONEST" | "LOW_AVBL_FIRST" | undefined,
    priorityGames,
    excludeGames,
  });
...
router.get("/miner/status", requireUser, (req, res) => {
  const user = (req as typeof req & { currentUser: { id: number } }).currentUser;
  ...
  void minerManager.ensureRunning(user.id).catch(() => undefined);
  res.json({ status: minerManager.getStatus(user.id) });
});
